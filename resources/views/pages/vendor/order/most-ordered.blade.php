        ...
        <div class="container-fluid">
            <div class="card">
                <div class="card-header">
                    <h4>Total Orders : {{ count($mostOrderedProducts) }}</h4>
                    <h4>Total Sold Amount :
                        {{
                            $mostOrderedProducts->filter(function($order){
                ...
                <div class="card-body table-responsive">
                    <table id="example1" class="table table-bordered table-hover">
                        ...
                        <tbody>
                        @foreach($mostOrderedProducts as $order)
                            <tr>
                                <td>{{ $loop->index + 1 }}</td>
                                <td><a href="{{ route('order.edit', $order->id) }}">{{ $order->code }}</a></td>
                                <td>{{ $order->name }}</td>
                                <td>{{ $order->phone }}</td>
                                <td><span class="badge badge-">{{ $order->title }}</span></td>
